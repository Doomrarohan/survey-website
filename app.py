...
.nav-link{font-size:13px;color:#666;font-family:'DM Sans',sans-serif;font-weight:500;padding:0 16px;display:flex;align-items:center;height:100%;border-bottom:2px solid transparent;cursor:pointer;transition:all .15s}
...
.hero-sub{font-family:'Playfair Display',Georgia,serif;font-size:17px;color:rgba(255,255,255,.6);font-style:italic}
...
.block-subtitle{font-size:13px;color:#888;padding-left:14px;margin-bottom:3px;font-family:'DM Sans',sans-serif}
.block-question{font-size:11px;color:#aaa;padding-left:14px;font-style:italic;font-family:'DM Sans',sans-serif}
...
.coming-soon-text{font-size:14px;color:#888;font-family:'DM Sans',sans-serif;max-width:500px;margin:0 auto;line-height:1.6}
...
.sample-label{font-size:11px;color:#aaa;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;font-family:'DM Sans',sans-serif}
...
def check_password():
    ...
    <div class="pw-screen"><div class="pw-box">
        <div class="pw-logo-text">Bain & Company</div><div class="pw-line"></div>
...
# Invisible clickable buttons overlaid on nav
...
div[data-testid="stHorizontalBlock"]:has(button[kind="secondary"]){
    position:absolute;top:54px;left:5vw;right:5vw;z-index:1000;display:flex;gap:0;height:54px;opacity:0}
...
def render_block(block, vert_color, prefix=""):
    st.markdown(
        f'<div class="block-card"><div class="block-bar" style="background:{vert_color}"></div>'
    ...
    if len(vnames) > 1:
        sel = st.selectbox("Split by", vnames, key=f"sel_{prefix}{block['id']}", label_visibility="collapsed")
    else:
    ...
    if sel and sel in views:
        ...
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False}, key=f"ch_{prefix}{block['id']}_{sel}")
        if ch.get("note"):
...
if current == "Overview":
    ...
    # Exec summary — placeholder media generic
    ...
    for v in verticals:
        ...
        cards_html += f'''<div class="vert-card">
            ...
            <div class="vert-card-desc">{v.get('card_desc','')}</div></div>'''
    cards_html += '</div>'
    ...
    # Sample survey cuts from gaming (single column, with dropdowns)
    st.markdown('<div class="exec-label">Sample survey cuts</div>', unsafe_allow_html=True)
    st.markdown('<div class="sample-label">Placeholder \u2014 showing gaming data as sample</div>', unsafe_allow_html=True)

    gaming = next((v for v in verticals if v["id"] == "gaming"), None)
    if gaming:
        for block in gaming.get("blocks", [])[:3]:
...
# ══════════════════════════════════════════
    ...
    st.markdown(f"""
    <div class="hero-full" style="padding:40px 5vw 36px"><div class="hero-inner">
    ...
    <div class="content">
        <div class="coming-soon">
            ...
            <div class="coming-soon-title">Work in progress</div>
